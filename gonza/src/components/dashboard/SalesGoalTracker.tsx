...
const SalesGoalTracker = () => {
  ...
  return (
    <div className="p-6 rounded-sm bg-white/40 dark:bg-white/[0.03] backdrop-blur-md border border-gray-100/50 dark:border-white/[0.05] shadow-xl">
      ...
      <div className="mb-4">
        <h2 className="text-lg font-bold text-gray-900 dark:text-white">Sales Goal Tracker</h2>
        <p className="text-sm text-gray-500 uppercase tracking-widest font-medium">Business: {user?.agency?.name || "Business Name"}</p>
      </div>
      ...
      <div className="flex p-1 bg-gray-100/50 dark:bg-black/20 rounded-sm mb-6 border border-gray-200/50 dark:border-white/5">
        {(["DAILY", "WEEKLY", "MONTHLY"] as const).map((p) => (
          <button
            key={p}
            onClick={() => setPeriod(p)}
            className={`flex-1 py-1.5 text-[10px] font-black uppercase tracking-wider transition-all rounded-sm ${
              period === p
      ...
      <div className="flex flex-col gap-2 mb-6">
        <div className="flex justify-between items-center py-2 border-b border-gray-100/50 dark:border-white/5">
          <p className="text-xs font-bold text-gray-500 uppercase tracking-widest">{periodLabel} Goal</p>
          <p className="text-sm font-black text-gray-900 dark:text-white">ugx {NumberFormatter.formatCurrency(currentGoal?.amount_target || 0)}</p>
        </div>
        <div className="flex justify-between items-center py-2 border-b border-gray-100/50 dark:border-white/5">
          <p className="text-xs font-bold text-gray-500 uppercase tracking-widest">Current Sales</p>
          <p className="text-sm font-black text-brand-primary dark:text-brand-accent">ugx {NumberFormatter.formatCurrency(currentSales || 0)}</p>
        </div>
      ...
      <div className="flex gap-2">
        ...
        <Button 
          ...
          className="rounded-sm bg-brand-primary text-white hover:bg-brand-primary-dark transition-all px-6 font-bold uppercase tracking-widest text-[10px]"
        >
      ...
      <div className="text-[10px] text-gray-400 italic font-medium leading-relaxed">
        Business Goal Tip: Setting realistic {periodLabel} targets ensures steady business growth and team motivation.
      </div>
    </div>
  );
};
